...
A = np.array([5, -2])  # Use floating-point numbers
B = np.array([-5, 5])  # Use floating-point numbers
C = np.array([-2, -5])  # Use floating-point numbers
...
def perpendicular(B, C, label):
    ...
    mid = midpoint(B, C)
    x_G = line_dir_pt(perpendicular, mid, 0, 1)
    plt.arrow(mid[0], mid[1], perpendicular[0], perpendicular[1], color='blue', head_width=0.4, head_length=0.4, label=label)
    plt.arrow(mid[0], mid[1], -perpendicular[0], -perpendicular[1], color='orange', head_width=0.4, head_length=0.4)
    return x_G

##Perpendicular Bisector
#x_G = perpendicular(A, B, 'OG')
